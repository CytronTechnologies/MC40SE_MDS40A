...
int main(void)
{
	...
	while (1) 
	{
		if(uc_skps(19)<50)   					//increase speed using joystik 2 push up
		{	lcd_clear_msg("+++ ");
			while(uc_skps(19)<50);
			if(i==1000)
			{i=1000;}
		...
		else if(uc_skps(19)>200)				//decrease speed when joystick 2 push down
		{	lcd_clear_msg("--- ");
		...
		else if(uc_skps(17)<50 && uc_skps(16)<135 && uc_skps(16)>110)		//control diraction using joystick 1
		{lcd_clear_msg("forw ");											//forward
		brush(PORT1, RUN, CCW, i);
		brush(PORT2, RUN, CCW, i);
		}
		else if(uc_skps(17)>150 && uc_skps(16)<150 && uc_skps(16)>100)		//reverse	
		{lcd_clear_msg("rev ");
		brush(PORT1, RUN, CW, i);
		brush(PORT2, RUN, CW, i);
		}
		else if(uc_skps(17)<100 && uc_skps(16)<100)							//left	
		{lcd_clear_msg("left ");
		brush(PORT1, RUN, CCW, i/2);
		brush(PORT2, RUN, CCW, i);
		}
		else if(uc_skps(17)<100 && uc_skps(16)>150)							//right
		{lcd_clear_msg("right ");
		brush(PORT1, RUN, CCW, i);
		brush(PORT2, RUN, CCW, i/2);
		}
		else if(uc_skps(16)==0)												//sharp left	
		{lcd_clear_msg("left2 ");
		brush(PORT1, RUN, CCW, i/3);
		brush(PORT2, RUN, CCW, i);
		}		
		else if(uc_skps(16)==255)											//sharp right	
		{lcd_clear_msg("right2 ");			
		brush(PORT1, RUN, CCW, i);
		brush(PORT2, RUN, CCW, i/3);
		}
		
		else if(uc_skps(17)>140 && uc_skps(16)<100)							//reverse left	
		{lcd_clear_msg("bleft2 ");
		brush(PORT1, RUN, CW, i/2);
		brush(PORT2, RUN, CW, i);
		}
		else if(uc_skps(17)>140 && uc_skps(16)>150)							//reverse right	
		{lcd_clear_msg("bright2 ");
		brush(PORT1, RUN, CW, i);
		brush(PORT2, RUN, CW, i/2);
		}
		else  																//stop	
		{lcd_clear_msg("stop ");
		brush(PORT1, BRAKE, CW, 0);
		brush(PORT2, BRAKE, CW, 0);
		}
...
//		if(uc_skps(17)<128 && uc_skps(19)<128)
//		{
////		lcd_clear_msg("front");
//			i=255-uc_skps(17); 
//			j=255-uc_skps(19);
...
//			set_pwm1(k);
//		}
//		else if(uc_skps(17)>128 && uc_skps(19)>128)
//		{
////		lcd_clear_msg("back ");
//		RE0 = 0;
//		RE1 = 0;
